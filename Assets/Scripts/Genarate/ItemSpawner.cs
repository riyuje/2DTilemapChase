...
public class ItemSpawner : MonoBehaviour
{
    ...
    public bool isSpawnItem;         //　生成処理を行うかどうかの判定値。true なら生成する
    ...
    [SerializeField]
    private int maxItemCount;        //  最大生成数の設定用の変数

    [SerializeField]
    private int currentItemCount;    //  現在の生成数を把握するための変数
    ...
    private IEnumerator ObserveTimer()
    {
        ...
        while (isSpawnItem && maxItemCount > currentItemCount)
        {
    ...
    private void RandomSpawnEnemy()
    {
        ...
        // 生成
        itemPickup item = SpawnItem(spawnPos);
        ...
        if (NavMesh.SamplePosition(item.transform.position, out NavMeshHit hit, 1.0f, NavMesh.AllAreas))
        {
            ...
            // 敵のターゲットとタイルマップの設定
            //item.Setup(target, tilemap);



            Debug.Log("アイテムの位置調整して配置");
        }
        else
        {
            Debug.Log("アイテムの配置不可");
    ...
    /// <returns></returns>
    public itemPickup SpawnItem(Vector3 spawnPos)
    {

        // 生成
        itemPickup item = Instantiate(ItemPrefab, spawnPos, ItemPrefab.transform.rotation);
